...
int XsQuaternion_compare(XsQuaternion const* thisPtr, XsQuaternion const* other, XsReal tolerance)
{
	...
	if (fuzzyCompare(thisPtr->m_data[0], other->m_data[0], tolerance) &&
		fuzzyCompare(thisPtr->m_data[1], other->m_data[1], tolerance) &&
		fuzzyCompare(thisPtr->m_data[2], other->m_data[2], tolerance) &&
		fuzzyCompare(thisPtr->m_data[3], other->m_data[3], tolerance))
		return 1;
	// add extra check for q == -q (negative-definite vs positive-definite comparison)
	return (fuzzyCompare(thisPtr->m_data[0], -other->m_data[0], tolerance) &&
			fuzzyCompare(thisPtr->m_data[1], -other->m_data[1], tolerance) &&
			fuzzyCompare(thisPtr->m_data[2], -other->m_data[2], tolerance) &&
			fuzzyCompare(thisPtr->m_data[3], -other->m_data[3], tolerance));
}
