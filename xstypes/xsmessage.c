...
#define XS_LEN_GPSLEVERARM				XS_LEN_GNSSLEVERARM	//!< \deprecated
...
#define XS_LEN_GPSSTATUS				XS_LEN_GNSSSTATUS	//!< \deprecated
...
/*! \brief Write a number of floating/fixed point values into to the data buffer, conversion depends on outputSettings
	...
	\param numValues number of values to be written
*/
void XsMessage_setDataFPValuesById(XsMessage* thisPtr, XsDataIdentifier dataIdentifier, XsReal const*data, XsSize offset, XsSize numValues)
{
	XsSize i;
	for (i=0; i<numValues; i++) {
		switch (dataIdentifier & XDI_SubFormatMask)
...
void XsMessage_toHexString(XsMessage const* thisPtr, XsSize maxBytes, XsString* resultValue)
{
	...
	XsString_resize(resultValue, maxBytes*3);
