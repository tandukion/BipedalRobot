...
Variant* createVariant(XsDataIdentifier id)
{
	...
	case XDI_UtcTime				:// 0x1010,
		return new XsUtcTimeVariant(id);
	...
	//case XDI_GpsGroup				:// 0x8800,		//!< \deprecated
	case XDI_GpsDop				:// 0x8830,		//!< \deprecated
		return new XsRawGpsDopVariant(id);
	case XDI_GpsSol				:// 0x8840,		//!< \deprecated
		return new XsRawGpsSolVariant(id);
	case XDI_GpsTimeUtc			:// 0x8880,		//!< \deprecated
		return new XsRawGpsTimeUtcVariant(id);
	case XDI_GpsSvInfo			:// 0x88A0,		//!< \deprecated
		return new XsRawGpsSvInfoVariant(id);
...
XsGpsPvtData* XsDataPacket_gpsPvtData(const XsDataPacket* thisPtr, XsGpsPvtData* returnVal)
{
	assert (returnVal);
	XsPressure pres;
	XsDataPacket_pressure(thisPtr, &pres);
	returnVal->m_pressure = (uint16_t)XsMath::doubleToLong(pres.m_pressure);
	returnVal->m_pressureAge = pres.m_pressureAge;

	returnVal->m_itow = GenericSimple<uint32_t>::get(thisPtr, XDI_Itow, 0);
	returnVal->m_gpsAge = GenericSimple<uint8_t>::get(thisPtr, XDI_GpsAge, 0);

	XsRawGpsSol sol;
	XsDataPacket_rawGpsSol(thisPtr, &sol);

	returnVal->m_latitude = sol.m_ecef_x;
	returnVal->m_longitude = sol.m_ecef_y;
	returnVal->m_height = sol.m_ecef_z;

	returnVal->m_veln = sol.m_ecef_vx;
	returnVal->m_vele = sol.m_ecef_vy;
	returnVal->m_veld = sol.m_ecef_vz;

	returnVal->m_hacc = sol.m_pacc;
	returnVal->m_vacc = sol.m_pacc;
	returnVal->m_sacc = sol.m_pacc;
...
{
	return	genericContains(thisPtr, XDI_GpsAge) &&
			genericContains(thisPtr, XDI_PressureAge) &&
			genericContains(thisPtr, XDI_BaroPressure) &&
			genericContains(thisPtr, XDI_Itow) &&
			genericContains(thisPtr, XDI_GpsSol);
}
...
{
	XsPressure pres(data->m_pressure, data->m_pressureAge);
	XsDataPacket_setPressure(thisPtr, &pres);

	GenericSimple<uint32_t>::set(thisPtr, data->m_itow, XDI_Itow);
	GenericSimple<uint8_t>::set(thisPtr, data->m_gpsAge, XDI_GpsAge);

	XsRawGpsSol sol;

	sol.m_ecef_x = data->m_latitude;
	sol.m_ecef_y = data->m_longitude;
	sol.m_ecef_z = data->m_height;
	sol.m_ecef_vx = data->m_veln;
	sol.m_ecef_vy = data->m_vele;
	sol.m_ecef_vz = data->m_veld;
	sol.m_pacc = data->m_hacc;

	XsDataPacket_setRawGpsSol(thisPtr, &sol);
}
...
{
	GenericSimple<uint32_t>::set(thisPtr, data->toInt(), XDI_DeviceId);
}
...
{
	return genericGet<XsUtcTime, XsUtcTimeVariant>(thisPtr, returnVal, XDI_UtcTime);
}
...
{
	genericSet<XsUtcTime, XsUtcTimeVariant>(thisPtr, data, XDI_UtcTime);
}
...
/*! \brief Returns a struct with XsRawGpsDop data
	\param returnVal Buffer for storage of requested value
	\return the supplied \a returnVal
	\deprecated Use the appropriate GNSS function instead
*/
XsRawGpsDop* XsDataPacket_rawGpsDop(const XsDataPacket* thisPtr, XsRawGpsDop* returnVal)
{
	return genericGet<XsRawGpsDop, XsRawGpsDopVariant>(thisPtr, returnVal, XDI_GpsDop);
}

/*! \brief Returns true if data item contains RawGpsDop, 0 otherwise
	\return non-zero if data item contains RawGpsDop, 0 otherwise
	\deprecated Use the appropriate GNSS function instead
*/
int XsDataPacket_containsRawGpsDop(const XsDataPacket* thisPtr)
{
	return genericContains(thisPtr, XDI_GpsDop);
}

/*! \brief Sets or updates the RawGpsDop value in the datapacket
	\param data The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsDop(XsDataPacket* thisPtr, const XsRawGpsDop* data)
{
	genericSet<XsRawGpsDop, XsRawGpsDopVariant>(thisPtr, data, XDI_GpsDop);
}
...
{
	return genericGet<XsRawGpsSol, XsRawGpsSolVariant>(thisPtr, returnVal, XDI_GpsSol);
}

/*! \brief Returns 1 if data item contains RawGpsSol, 0 otherwise
	\return non-zero if data item contains RawGpsSol, 0 otherwise
	\deprecated Use the appropriate GNSS function instead
*/
int XsDataPacket_containsRawGpsSol(const XsDataPacket* thisPtr)
{
	return genericContains(thisPtr, XDI_GpsSol);
}

/*! \brief Sets or updates the RawGpsSol value in the datapacket
	\param data The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsSol(XsDataPacket* thisPtr, const XsRawGpsSol* data)
{
	genericSet<XsRawGpsSol, XsRawGpsSolVariant>(thisPtr, data, XDI_GpsSol);
}
...
{
	return genericGet<XsRawGpsTimeUtc, XsRawGpsTimeUtcVariant>(thisPtr, returnVal, XDI_GpsTimeUtc);
}

/*! \brief Returns 1 if data item contains RawGpsTimeUtc, 0 otherwise
	\return non-zero if data item contains RawGpsTimeUtc, 0 otherwise
	\deprecated Use the appropriate GNSS function instead
*/
int XsDataPacket_containsRawGpsTimeUtc(const XsDataPacket* thisPtr)
{
	return genericContains(thisPtr, XDI_GpsTimeUtc);
}

/*! \brief Sets or updates the RawGpsTimeUtc value in the datapacket
	\param data The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsTimeUtc(XsDataPacket* thisPtr, const XsRawGpsTimeUtc* data)
{
	genericSet<XsRawGpsTimeUtc, XsRawGpsTimeUtcVariant>(thisPtr, data, XDI_GpsTimeUtc);
}

/*! \brief Returns a struct with RawGpsSvInfo data
	\param returnVal Buffer for storage of requested value
	\return the supplied \a returnVal
	\deprecated Use the appropriate GNSS function instead
*/
...
{
	return genericGet<XsRawGpsSvInfo, XsRawGpsSvInfoVariant>(thisPtr, returnVal, XDI_GpsSvInfo);
}

/*! \brief Returns 1 if data item contains RawGpsSvInfo, 0 otherwise
	\return 1 if data item contains RawGpsSvInfo, 0 otherwise
	\deprecated Use the appropriate GNSS function instead
*/
int XsDataPacket_containsRawGpsSvInfo(const XsDataPacket* thisPtr)
{
	return genericContains(thisPtr, XDI_GpsSvInfo);
}

/*! \brief Sets or updates the RawGpsSvInfo value in the datapacket
	\param r The new value that should be added to the packet
*/
void XsDataPacket_setRawGpsSvInfo(XsDataPacket* thisPtr, const XsRawGpsSvInfo* r)
{
	genericSet<XsRawGpsSvInfo, XsRawGpsSvInfoVariant>(thisPtr, r, XDI_GpsSvInfo);
}
...
{
	if (XsDataPacket_containsRawGnssSatInfo(thisPtr))
		return genericGet<XsRawGnssSatInfo, XsRawGnssSatInfoVariant>(thisPtr, returnVal, XDI_GnssSatInfo);
	else if (XsDataPacket_containsRawGpsSvInfo(thisPtr))
	{
		XsSize i;
		XsRawGpsSvInfo svInfo;
		(void)XsDataPacket_rawGpsSvInfo(thisPtr, &svInfo);
		returnVal->m_itow = svInfo.m_itow;
		returnVal->m_numSvs = svInfo.m_nch;
		for (i = 0; i < returnVal->m_numSvs; i++)
		{
			returnVal->m_satInfos[i].m_gnssId = 0;	// Gps
			returnVal->m_satInfos[i].m_svId = svInfo.m_svInfos[i].m_svid;
			returnVal->m_satInfos[i].m_cno = svInfo.m_svInfos[i].m_cno;
			returnVal->m_satInfos[i].m_flags = svInfo.m_svInfos[i].m_qi;
			returnVal->m_satInfos[i].m_flags |= (svInfo.m_svInfos[i].m_flags & 0x1) << 3;
			if ((svInfo.m_svInfos[i].m_flags & 0x10) == 0)
				returnVal->m_satInfos[i].m_flags |= 0x10;	// healthy
			else
				returnVal->m_satInfos[i].m_flags |= 0x20;	// SV is unhealthy / shall not be used
			returnVal->m_satInfos[i].m_flags |= (svInfo.m_svInfos[i].m_flags & 0x2) << 5;
		}
	}
	return returnVal;
}
