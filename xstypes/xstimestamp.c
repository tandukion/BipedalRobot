...
void XsTimeStamp_toUtcTime(struct XsTimeStamp* thisPtr, struct XsUtcTime* utc)
{
	...
	utc->m_day = (uint8_t) tmUtc.tm_mday;
	utc->m_hour = (uint8_t) tmUtc.tm_hour;
	utc->m_minute = (uint8_t) tmUtc.tm_min;
	utc->m_month = (uint8_t) tmUtc.tm_mon + 1;
	utc->m_nano = (uint32_t)((thisPtr->m_msTime % 1000) * 1e6);
	utc->m_second = (uint8_t) tmUtc.tm_sec;
	utc->m_year = (uint16_t) (tmUtc.tm_year + 1900);
	utc->m_valid = 1;
}
